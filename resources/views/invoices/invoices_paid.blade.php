...
@section('css')
    ...
    <!-- إضافة تعريف الفئة text-success -->
    <style>
        .text-success {
                            ...
                            <tbody>
                                ...
                                @foreach ($invoices as $invoice)
                                    ...
                                    <tr>
                                        ...
                                        <td><a href="{{ url('InvoicesDetails') }}/{{ $invoice->id }}">{{ $invoice->section->sectionName }}</a></td>
                                        <td>{{ $invoice->discount }}</td>
                                        <td>{{ $invoice->rateVAT }}</td>
                                        <td>{{ $invoice->valueVAT }}</td>
                                        <td>{{ $invoice->total }}</td>
                                        <td>
                                            @if ($invoice->valuesStatus == 1)
                                                <span class="text-success">{{ $invoice->status }}</span>
                                            @elseif($invoice->valuesStatus == 2)
                                                <span class="text-success">{{ $invoice->status }}</span>
                                            @else
                                                <span class="text-success">{{ $invoice->status }}</span>
                                            @endif
                                        ...
                                        <td>
                                            <div class="dropdown">
                                                ...
                                                <div class="dropdown-menu tx-13">
                                                    ...
                                                    <a class="dropdown-item" href="#" data-invoiceId="{{ $invoice->id }}" data-toggle="modal" data-target="#delete_invoice"><i class="text-danger fas fa-trash-alt"></i>&nbsp;&nbsp;حذف الفاتورة</a>
                                                    <a class="dropdown-item" href="{{ URL::route('Status_show', [$invoice->id]) }}"><i class="text-success fas fa-money-bill"></i>&nbsp;&nbsp;تغير حالة الدفع</a>
                                                    <a class="dropdown-item" href="#" data-invoiceId="{{ $invoice->id }}" data-toggle="modal" data-target="#Transfer_invoice"><i class="text-warning fas fa-exchange-alt"></i>&nbsp;&nbsp;نقل الى الارشيف</a>
                                                </div>
                                            </div>
                                        </td>
                                    </tr>
